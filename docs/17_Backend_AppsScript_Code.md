...
Jalankan fungsi `setupSpreadsheet()` sekali dari editor Google Apps Script untuk membuat semua sheet, header, dan akun admin awal. Fungsi ini aman untuk dijalankan ulang karena tidak menghapus data yang sudah ada.
...
function seedInitialData_() {
  ...
  const tahunAjaranSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('TahunAjaran');
